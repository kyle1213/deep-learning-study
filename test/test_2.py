...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x2 = self.conv0_layer(pic)
        x2 = self.maxpool(x2)
        shortcut = x2

        x2 = self.conv1_layer(x2)
        x2 = self.relu(x2 + shortcut)
        shortcut = x2

        x2 = self.conv1_layer(x2)
        x2 = self.relu(x2 + shortcut)
        shortcut = self.conv1(x2)
        shortcut = self.batch1(shortcut)

        x2 = self.conv2_1_layer(x2)
        x2 = self.relu(x2 + shortcut)
        shortcut = x2

        x2 = self.conv2_2_layer(x2)
        x2 = self.relu(x2 + shortcut)
        shortcut = self.conv2(x2)
        shortcut = self.batch2(shortcut)

        x2 = self.conv3_1_layer(x2)
        x2 = self.relu(x2 + shortcut)
        shortcut = x2

        x2 = self.conv3_2_layer(x2)
        x2 = self.relu(x2 + shortcut)
